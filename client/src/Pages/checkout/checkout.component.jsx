...
const CheckoutPage = ({ cartItems, total }) => {
  return (
    <div className="checkout-page">
      <div className="checkout-title">ตะกร้าสินค้า</div>
      ...
      {cartItems.length > 0 ? (
        cartItems.map((item) => <CheckoutItems cartItem={item} key={item.id} />)
      ) : (
        <h4>ไม่มีสินค้าในตะกร้า</h4>
      )}
      <div className="checkout-footer">
        <span>รวมราคาทั้งหมด</span>
        <span className="total">{currencyFormatter.format(total)}</span>
      </div>
      ...
      <div className="demo-container">
        ...
        <span className="demo-number">
          หมายเลขบัตร{" "}
          <span className="demo-number-text">4242 4242 4242 4242</span>
        </span>
        <div className="demo-other">วันหมดอายุ : 05/24 CVV : 999</div>
      </div>
    </div>
  );
};
